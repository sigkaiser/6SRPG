...
export const getSchema = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/users/data/schema`);
    const data = await response.json();
    if (!response.ok) {
      return { success: false, message: data.message || 'Failed to fetch schema.' };
    }
    return { success: true, data };
  } catch (error) {
    console.error('Get Schema API error:', error);
    return { success: false, message: error.message || 'A network error occurred while fetching schema.' };
  }
}

export const getExercises = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/users/data/exercises`);
    const data = await response.json();
    if (!response.ok) {
      return { success: false, message: data.message || 'Failed to fetch exercises.' };
    }
    return { success: true, data };
  } catch (error) {
    console.error('Get Exercises API error:', error);
    return { success: false, message: error.message || 'A network error occurred while fetching exercises.' };
  }
}
